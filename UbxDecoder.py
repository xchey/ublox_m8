...
if __name__ == '__main__':
    raw = rtk.raw_t()
    rtk.init_raw(raw, rtk.STRFMT_UBX)

    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('port', type=str, default=None, help="Serial com port")

    parser.add_argument('--min', '-m', type=float, default=3, dest='min',
                        help="Capture ublox raw data time duration")
    parser.add_argument('--show', '-sh', type=bool, default=False, dest='show',
                        help="Just show raw obs")
    ...
    out_raw = open('test_raw.txt', 'w')
    out_eph = open('test_eph.txt', 'w')

    out_raw.write(u2r.rinex3_header(runby="Chey's", marker='', observer=' ', agency=' ',
                                    receiver='NEO-M8T', rxtype='', version='2.01', antenna=' '))

    out_eph.write(u2r.rinex3_nav_header(runby="Chey's"))

    if minutes >= 60:
        h = int(minutes/60)
        m = int(minutes-h*60)
        s = int((minutes-h*60-m)*60)
    else:
        h = 0
        m = int(minutes)
        s = int((minutes-m)*60)
        ...
        if not show:
            try:
                ...
                for i in range(data_len):
                    ret = rtk.input_ubx(raw, unpack('<B', data[i].to_bytes(1, 'little'))[0])
                    if ret <= 0:
                        continue
                    elif ret & rtk.ID_RXMRAWX == rtk.ID_RXMRAWX:
                        u2r.rto_obs(out_raw, raw)
                    elif ret & rtk.ID_RXMSFRBX == rtk.ID_RXMSFRBX:
                        u2r.rto_eph(out_eph, raw)
            except TypeError:
                continue
        else:
            try:
